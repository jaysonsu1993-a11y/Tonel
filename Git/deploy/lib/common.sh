#!/bin/bash
# Common functions for all Git/deploy/*.sh scripts.
...
DEPLOY_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"   # Git/deploy
GIT_DIR="$(cd "$DEPLOY_DIR/.." && pwd)"                         # Git/
REPO_ROOT="$(cd "$GIT_DIR/.." && pwd)"                          # repo root
