...
A running log of real incidents that shaped [`DEPLOY_SCRIPTING_STANDARDS.md`](../docs/DEPLOY_SCRIPTING_STANDARDS.md).
...
`/opt/tonel/` and shipped `Git/deploy/` as the official deploy path.
...
`Git/web/package-lock.json` modified. The next deploy script that
...
1. Decide if the failure mode generalizes. If yes, add a new rule
   (R-something) to [DEPLOY_SCRIPTING_STANDARDS.md](../docs/DEPLOY_SCRIPTING_STANDARDS.md).
