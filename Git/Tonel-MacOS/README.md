...
* **When app changes ride along with a `main` release** (or affect
  ...
  pipeline — bump umbrella version, write `Git/CHANGELOG.md` entry,
  commit as `release: vX.Y.Z`, tag, push. The pre-push hook
  (`Git/scripts/hooks/pre-push`) blocks any non-`release:` commit
  on main; install with `Git/scripts/install-hooks.sh` once per clone.
