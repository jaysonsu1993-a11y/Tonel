// CreateRoomViewController.mm — Create room dialog
...
- (void)buildUI {
    ...
    // Title
    NSTextField* title = [NSTextField labelWithString:@"创建房间"];
    ...
    _roomCodeField.placeholderString = @"自定义房间号（如 S1-001）";
    ...
    NSTextField* pwLabel = [self rowLabel:@"密码（可选）"];
    [card addSubview:pwLabel];

    _passwordField = [[NSSecureTextField alloc] init];
    _passwordField.translatesAutoresizingMaskIntoConstraints = NO;
    _passwordField.placeholderString = @"不设密码则留空";
    ...
    confirmBtn.title = @"创建";
    confirmBtn.fillColor = S1ThemeAccentBlue();
    confirmBtn.contentTintColor = S1ThemeTextPrimary();
...
- (void)confirmTapped:(id)sender {
    ...
    if (code.length == 0) {
        // Generate a random 4-digit code if left blank
        code = [NSString stringWithFormat:@"%04d", arc4random_uniform(9000) + 1000];
    }
