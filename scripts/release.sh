...
#   4. deploy/server.sh              (rsync + remote build + pm2 reload + nginx reload)
#   5. deploy/web.sh                 (vite build + wrangler pages deploy)
#   6. deploy/health.sh              (verify everything is live)
#
# Usage:
...
if [ "${SKIP_PRETEST:-0}" = "1" ]; then
    ...
    log "[0/6] pre-release smoke (scripts/pretest.sh)"
